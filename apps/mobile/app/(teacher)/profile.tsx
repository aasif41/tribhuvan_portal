  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.put('/users/profile', {
        ...
        teacher: { department: formData.department, designation: formData.designation },
      });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.headerRow}>
        <PageHeader title="My Profile" subtitle="Your faculty information" />
        <TouchableOpacity
          onPress={() => {
        ...
        >
          <Text style={[styles.editBtnText, isEditing && styles.cancelBtnText]}>{isEditing ? 'Cancel' : 'Edit Profile'}</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.avatarSection}>
          ...
          {isEditing ? (
            ...
            <View style={styles.nameSection}>
              ...
              <View style={{ marginTop: 8 }}><Badge variant="success">{user?.status}</Badge></View>
            </View>
          )}
        </View>
      </Card>
      ...
      <Card>
        ...
        <InfoRow label="Employee ID (Read-only)" value={teacher?.employeeId || 'N/A'} readonly />
        <InfoRow label="Status" value="Active Faculty Member" readonly />
        {isEditing ? (
          <>
            <EditRow label="Department" value={formData.department} onChangeText={(v: string) => setFormData({ ...formData, department: v })} />
            <EditRow label="Designation" value={formData.designation} onChangeText={(v: string) => setFormData({ ...formData, designation: v })} />
          </>
        ) : (
          <>
...
function EditRow({ label, value, onChangeText }: any) {
  return (
    <View style={infoS.editRow}>
      <Text style={infoS.label}>{label}</Text>
      <TextInput style={infoS.editInput} value={value} onChangeText={onChangeText} />
    </View>
  );
}
